...
// CONFIGURACI├ôN DE L├ìMITES POR PLAN
const PLAN_LIMITS = {
  free: {
...
const qrPhrases = [
  '┬íEscanea y d├®janos tu opini├│n!',
  'Ay├║danos a mejorar',
  'Cu├®ntanos tu experiencia',
  '┬┐C├│mo fue tu visita?',
  '┬íQueremos saber de ti!',
];

type DatePreset = 'all' | 'month' | 7 | 30 | 90 | 'custom';

const statsDatePresets = [
  { label: '7 d├¡as', days: 7 as const, icon: Clock },
  { label: '30 d├¡as', days: 30 as const, icon: Calendar },
  { label: '90 d├¡as', days: 90 as const, icon: CalendarDays },
...
export function DashboardPage({ user, onLogout, themeProps }: DashboardPageProps) {
    ...
    try {
      ...
      if (signInError) {
        if (signInError.status === 429) {
          toast.error('L├¡mite de intentos alcanzado. Espera unos minutos.');
        } else {
  ...
  const handleExportExcel = useCallback(() => {
    ...
    setTimeout(() => {
      const headers = ['Fecha', 'Tipo', 'Comentario', 'Satisfacci├│n'];
        ...
        <html>
          ...
          <body>
            <div class="header">
              ...
              <p class="date-range">Per├¡odo: ${dateRangeText}</p>
            </div>
